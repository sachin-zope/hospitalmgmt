...
<head>
	...
	<script src="js/mtp_register.js"></script>
...
<body>
	...
	<div class="container" ng-controller="MTPRegisterCtrl">
		...
		<div class="row" style="padding-bottom: 20px;">
			...
			<form class="form-inline">
				...
				<button type="button" class="btn btn-primary"
					ng-click="loadMTPRegister()">Submit</button>
			</form>
		</div>
		</div>
		<div class="row">
			<div>
				<table class="table table-bordered">
					...
					<tbody>
						<tr
							ng-repeat="mtpr in allRecords | offset: currentPage*itemsPerPage | limitTo: itemsPerPage">
							<td>{{mtpr.mtpSerialNo}}</td>
							<td align="center">{{mtpr.mtpMonthlySerialNo}}</td>
							<td>DOA:{{mtpr.admitDate}}<br>DOT: {{mtpr.operationDate}}<br>DOD:{{mtpr.dischargeDate}}</td>
							<td>{{mtpr.pName}}<br>{{mtpr.pAddress}} &nbsp; &nbsp;
								&nbsp; &nbsp; &nbsp; &nbsp; {{mtpr.gender}}/{{mtpr.age}}<br>{{mtpr.married}}</td>
							<td>{{mtpr.durationOfPregnancy}} W</td>
							
							<td>Indication: {{mtpr.mindication}}<br>Method: {{mtpr.procedure}}<br>With: {{mtpr.alongWith}} 
							<span ng-if="mtpr.batchNo"><br>Batch No: {{mtpr.batchNo}} </span></td>
							<td>{{mtpr.opinionGivenBy}}</td>
							<td>{{mtpr.doneByDr}}</td>
							<td>M: {{mtpr.mChildrens}}<br>F: {{mtpr.fChildrens}}</td>
						...
						<tr ng-hide="allRecords.length">
							<td style="text-align:center; color:#ff0000;" colspan="10" >No records found!</td>
						</tr>
					</tbody>
				</table>
